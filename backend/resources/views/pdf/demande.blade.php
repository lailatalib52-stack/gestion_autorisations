...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body { font-family: 'DejaVu Sans', Arial, sans-serif; font-size: 12px; color: #222; background: #fff; padding: 20px; }
        .header { background: #1a3c5e; color: #fff; padding: 15px 25px; margin-bottom: 10px; width: 100%; }
        .header table { width: 100%; border: none; }
        .header h1 { font-size: 18px; font-weight: 700; letter-spacing: 1px; color: #fff; margin: 0; }
        .header .ref { font-size: 10px; opacity: 0.9; margin-top: 2px; color: #fff; }
        .logo { font-size: 24px; font-weight: 900; color: #f0c040; letter-spacing: 2px; }
        .badge { display: inline-block; padding: 4px 12px; border-radius: 15px; font-weight: 700; font-size: 11px; }
        ...
        .section { margin: 15px 10px 0; }
        .section-title { font-size: 12px; font-weight: 700; color: #1a3c5e; text-transform: uppercase; letter-spacing: 1px; border-bottom: 2px solid #1a3c5e; padding-bottom: 3px; margin-bottom: 10px; }
        .label { font-size: 10px; color: #666; text-transform: uppercase; margin-bottom: 2px; }
        .value { font-size: 12px; color: #222; font-weight: 500; }
        .comment-box { background: #f8f9fa; border-left: 3px solid #1a3c5e; padding: 8px 12px; border-radius: 0 4px 4px 0; margin-top: 5px; font-style: italic; color: #444; font-size: 11px; }
        .comment-box.manager { border-left-color: #0a5c36; }
        .footer-table { margin-top: 30px; width: 100%; border-collapse: collapse; }
        .footer-table td { width: 33.33%; vertical-align: top; padding: 0 10px; }
        .signature-box { border: 1px solid #ccc; border-radius: 4px; padding: 8px; text-align: center; height: 100px; }
        .signature-box .sig-label { font-size: 10px; font-weight: 700; color: #1a3c5e; text-transform: uppercase; margin-bottom: 45px; border-bottom: 1px solid #eee; padding-bottom: 3px; }
        .watermark { text-align: center; color: #999; font-size: 9px; margin-top: 15px; }
        table.info { width: 100%; border-collapse: collapse; }
        table.info td { padding: 5px 8px; border: 1px solid #e0e0e0; font-size: 11px; }
        table.info tr:nth-child(even) td { background: #f9fafb; }
        table.info td:first-child { font-weight: 600; color: #1a3c5e; width: 35%; }
    </style>
</head>
<body>
...
<div class="section">
    ...
    <table class="info">
        ...
        <tr><td>Département / Poste</td><td>{{ $demande->employe->departement ?? '-' }} / {{ $demande->employe->poste ?? '-' }}</td></tr>
...
<div class="section">
    ...
    <table class="info">
        ...
        <tr><td>Période</td><td>Du {{ $demande->date_debut->format('d/m/Y') }} au {{ $demande->date_fin->format('d/m/Y') }} ({{ $demande->duree }} jour(s))</td></tr>
